#!/usr/bin/env python3
# Blockchain-style integrity broker for Pi 3 (LED-based)
#
# Expected payload per MQTT message from pi3_blockchain_led.py:
#   [led_frame_bytes][index(4)][hash_time_us(4)][prev_block_hash(32)][block_hash(32)]
#
# The broker recomputes frame_hash = SHA256(led_frame_bytes) and
# block_hash' = SHA256(prev_block_hash || frame_hash), verifies the
...
parser = argparse.ArgumentParser(description="Pi 3 blockchain integrity broker with stress-labelled output.")
parser.add_argument(
    "-s",
    "--stress",
    type=int,
...
# Data storage
metrics_buffer = []  # rows: [Index, FrameBytes, HashTime_uS, VerifyTime_uS, PrevHash_OK, Valid]
hash_times = []
verify_times = []
failures = 0
...
print(f"--- PI3 BLOCKCHAIN BENCHMARK RUN {RUN_ID} READY ({STRESS_LABEL}) ---")
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # Recompute frame hash and block hash
        v_start = time.perf_counter()
        frame_hash = hashlib.sha256(frame_bytes).digest()
        recomputed_block_hash = hashlib.sha256(prev_hash + frame_hash).digest()
        ...
        if index % 50 == 0:
            print(
                f"Block #{index:<5} | Size: {len(frame_bytes)} B | "
...
def finalize_benchmark():
    ...
    print(f"Avg Hash (Pi 3):   {avg_hash:.2f} us")
