...
# --- CONFIGURATION ---
MQTT_BROKER = "localhost" # Use localhost if running on the same laptop as broker
TOPIC = "cam"
SUMMARY_FILE = "benchmark_summary.csv"

# --- 1. NUMBERED FILE SETUP ---
...
OUTPUT_VIDEO = get_next_filename("final_signed_stream", ".h264")
RAW_LOG_FILE = get_next_filename("raw_packet_data", ".csv")
...
print(f"Logging Video to: {OUTPUT_VIDEO}")
print(f"Logging CSV to:   {RAW_LOG_FILE}")
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # Live Feed (Sampled every 100 chunks)
        if chunk_id % 100 == 0:
...
def finalize_benchmark():
    ...
    max_sign_index = sign_times.index(max_sign) + 1 
    ...
    print(f"  Max Spike:    {max_sign} us (Chunk #{max_sign_index})")
    print("-" * 50)
    print(f"BROKER VERIFYING (Laptop):")
    print(f"  Average:      {avg_verify:.2f} us")
    print(f"  Max Spike:    {max_verify:.2f} us (Chunk #{max_verify_index})")
    print("="*50)
    ...
    # --- SAVE SUMMARY ROW (Append Mode) ---
    file_exists = os.path.exists(SUMMARY_FILE)
    with open(SUMMARY_FILE, "a", newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["Timestamp", "Video_File", "Total_Chunks", "Avg_Sign_uS", "Max_Sign_uS", "Avg_Verify_uS", "Max_Verify_uS"])
